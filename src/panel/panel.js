...
(function () {
  ...
  // ============================================================
  // VERIFICAR CONTENT SCRIPT — inyectar si no responde
  // ============================================================
  ...
  async function checkContentScript() {
    ...
    // Intentar PING al content script
    chrome.tabs.sendMessage(tabs[0].id, { type: "PING" }, async (response) => {
      if (chrome.runtime.lastError || !response?.success) {
        // Content script no está — inyectarlo programáticamente
        try {
          ...
          // Esperar un momento y pedir el nombre del chat
          setTimeout(() => pollChatName(tabs[0].id), 800);
        } catch (e) {
          console.warn("No se pudo inyectar:", e.message);
        }
      } else {
        // Ya estaba corriendo — pedir chat name
  ...
  function pollChatName(tabId) {
    chrome.tabs.sendMessage(tabId, { type: "GET_CHAT" }, (response) => {
      if (response?.success && response.chatName) {
        updateChatName(response.chatName);
      }
    });
  }

  function showNotWhatsApp() {
    errorText.textContent = "Abrí WhatsApp Web (web.whatsapp.com) en esta pestaña.";
  ...
  async function analyzeConversation(altTone = null) {
    ...
    try {
      // Intentar inyectar si hace falta antes de pedir el chat
  ...
  // ============================================================
  // INYECTAR CONTENT SCRIPT SI NO ESTÁ
  // ============================================================
        ...
        chrome.tabs.sendMessage(tabs[0].id, { type: "PING" }, async (response) => {
          if (chrome.runtime.lastError || !response?.success) {
            try {
              ...
              await new Promise(r => setTimeout(r, 600)); // esperar que se inicialice
            } catch (e) {
